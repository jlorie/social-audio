...
exports.resizeArgs = function(options) {
  ...
  if (opt.width || opt.height) {
    ...
    if (opt.height === 0) args.push(String(opt.width) + shrinkSufix);
    else if (opt.width === 0) args.push('x' + String(opt.height) + shrinkSufix);
    else args.push(String(opt.width) + 'x' + String(opt.height) + shrinkSufix);
  }
